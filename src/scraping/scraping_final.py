...
def parse_car(car_id):
    ...
    try:
        ...
        if r.status_code in (403, 429, 503) or len(r.text) < 500:
            print(f"  ⚠ status={r.status_code} — refreshing session")
            session = make_session()
            time.sleep(5)
            r = session.get(url, timeout=12)
...
def main():
    ...
    OUT_PATH = "data/raw/car_listings_final_scraping.csv"
    ...
    print("STARTING ID CRAWL (3184999 → 3175000)...\n")
    ...
    for car_id in range(start_id, end_id, -1):
        ...
        time.sleep(random.uniform(0.5, 2.) if not car else random.uniform(0.5, 1.0))
