...
# =========================
def main():
    start_id = 3199999   # ← new range
    end_id   = 3185000   # ← new range

    # ── saves to a SEPARATE file so existing data is never touched ───────
    OUT_PATH = "data/raw/car_listings_raw_new.csv"
    ...
    print("STARTING ID CRAWL (3199999 → 3185000)...\n")
    for car_id in range(start_id, end_id, -1):
        ...
        time.sleep(random.uniform(0.8, 5) if not car else random.uniform(0.8, 1.5))

        # checkpoint every 50 found — saves to the NEW file only
        if found > 0 and found % 50 == 0:
            os.makedirs("data/raw", exist_ok=True)
            pd.DataFrame(data).to_csv(OUT_PATH, index=False)
            print(f"💾 checkpoint saved → {OUT_PATH}")

    # final save → NEW file only, never touches car_listings_raw.csv
    os.makedirs("data/raw", exist_ok=True)
    pd.DataFrame(data).drop_duplicates(subset="url").to_csv(OUT_PATH, index=False)
    print(f"\nDONE — found: {found}, missed: {miss}")
    print(f"Saved to {OUT_PATH} — now run merge.py to combine with existing data")
